-- Snapshot v5 and event v6 record Game 1 preparation, reactions, draws and entropy.
-- Prior codecs remain readable. Official history and manifests are not rewritten.
ALTER TABLE games DROP CONSTRAINT games_snapshot_version_supported,
    ADD CONSTRAINT games_snapshot_version_supported CHECK (snapshot_version IN (1, 2, 3, 4, 5));
ALTER TABLE game_events DROP CONSTRAINT game_events_version_supported,
    ADD CONSTRAINT game_events_version_supported CHECK (event_version IN (1, 2, 3, 4, 5, 6));

CREATE FUNCTION valid_effect_path_v6(path JSONB)
...
RETURN jsonb_typeof(path) = 'array' AND jsonb_array_length(path) <= 32
    AND NOT EXISTS (SELECT 1 FROM jsonb_array_elements(path) AS segment
        WHERE segment IS DISTINCT FROM '{"type":"reaction_effect"}'::jsonb
          AND valid_effect_path_segment_v4(segment) IS NOT TRUE);

CREATE FUNCTION valid_effect_cursor_v6(cursor_value JSONB)
RETURNS BOOLEAN LANGUAGE sql IMMUTABLE
RETURN cursor_value = jsonb_build_object('rule_id', cursor_value -> 'rule_id', 'path', cursor_value -> 'path')
    ...
    AND valid_effect_path_v6(cursor_value -> 'path') IS TRUE;

CREATE FUNCTION valid_queued_effect_v6(frame JSONB)
RETURNS BOOLEAN
LANGUAGE plpgsql
IMMUTABLE
AS $$
BEGIN
    IF jsonb_typeof(frame) IS DISTINCT FROM 'object'
       ...
       OR valid_effect_cursor_v6(frame -> 'cursor') IS NOT TRUE
    THEN
        RETURN FALSE;
    END IF;
...
CREATE FUNCTION valid_effect_continuation_v6(continuation JSONB)
RETURNS BOOLEAN
LANGUAGE plpgsql
IMMUTABLE
AS $$
BEGIN
    IF jsonb_typeof(continuation) IS DISTINCT FROM 'object'
       OR valid_effect_cursor_v6(continuation -> 'choice_cursor') IS NOT TRUE
    ...
    RETURN continuation = jsonb_build_object(
        ...
        AND NOT EXISTS (
            ...
            WHERE valid_queued_effect_v6(queued) IS NOT TRUE
        );
END;
$$;

CREATE FUNCTION valid_pending_effect_choice_v6(choice JSONB)
RETURNS BOOLEAN
LANGUAGE plpgsql
IMMUTABLE
AS $$
BEGIN
    IF jsonb_typeof(choice) IS DISTINCT FROM 'object'
       ...
       OR valid_effect_continuation_v6(choice -> 'continuation') IS NOT TRUE
...
CREATE FUNCTION valid_decision_point_v6(decision JSONB)
RETURNS BOOLEAN
LANGUAGE plpgsql
IMMUTABLE
AS $$
    ...
    CASE decision ->> 'type'
        ...
        WHEN 'effect_choice' THEN
            RETURN decision = jsonb_build_object(
                ...
                AND valid_pending_effect_choice_v6(decision -> 'choice');
        ELSE
            RETURN FALSE;
    END CASE;
END;
...
CREATE FUNCTION valid_engine_control_v6(control JSONB)
RETURNS BOOLEAN
LANGUAGE plpgsql
IMMUTABLE
AS $$
BEGIN
    IF control IS DISTINCT FROM jsonb_build_object(
       ...
       OR EXISTS (
            ...
            WHERE valid_queued_effect_v6(frame) IS NOT TRUE
       )
       OR valid_decision_point_v6(control -> 'decision_point') IS NOT TRUE
    THEN
        RETURN FALSE;
    END IF;
...
CREATE FUNCTION valid_effect_outcome_v6_base(effect JSONB)
RETURNS BOOLEAN
LANGUAGE plpgsql
IMMUTABLE
AS $$
...
BEGIN
    ...
    CASE effect ->> 'type'
        WHEN 'random_sampled' THEN
            RETURN effect = jsonb_build_object('type', 'random_sampled', 'rule_id', effect -> 'rule_id',
                'upper_exclusive', effect -> 'upper_exclusive', 'result', effect -> 'result')
                AND jsonb_typeof(effect -> 'upper_exclusive') = 'number'
                AND effect ->> 'upper_exclusive' ~ '^[1-9][0-9]*$'
                AND (effect ->> 'upper_exclusive')::NUMERIC <= 4294967295
                AND jsonb_typeof(effect -> 'result') = 'number'
                AND effect ->> 'result' ~ '^(0|[1-9][0-9]*)$'
                AND (effect ->> 'result')::NUMERIC < (effect ->> 'upper_exclusive')::NUMERIC;
        WHEN 'drawing_blocked' THEN
            RETURN effect = jsonb_build_object('type', 'drawing_blocked', 'rule_id', effect -> 'rule_id',
                'target_id', effect -> 'target_id', 'target_position', effect -> 'target_position')
                AND jsonb_typeof(effect -> 'target_id') = 'string'
                AND octet_length(effect ->> 'target_id') BETWEEN 1 AND 256
                AND jsonb_typeof(effect -> 'target_position') = 'number'
...
CREATE FUNCTION valid_effect_outcome_v6(effect JSONB)
RETURNS BOOLEAN
LANGUAGE plpgsql
IMMUTABLE
AS $$
BEGIN
    IF valid_effect_outcome_v6_base(effect) IS NOT TRUE
...
CREATE FUNCTION valid_turn_step_v6(step JSONB)
RETURNS BOOLEAN
LANGUAGE plpgsql
IMMUTABLE
AS $$
BEGIN
    ...
    RETURN NOT EXISTS (
        ...
        WHERE valid_effect_outcome_v6(effect) IS NOT TRUE
    );
END;
$$;

CREATE FUNCTION valid_snapshot_effect_entity_v5(entity JSONB)
RETURNS BOOLEAN
LANGUAGE plpgsql
IMMUTABLE
AS $$
...
BEGIN
    ...
    IF entity ? 'drawing_blocked' THEN
        expected := expected || jsonb_build_object('drawing_blocked', entity -> 'drawing_blocked');
        IF entity -> 'drawing_blocked' IS DISTINCT FROM 'true'::jsonb OR entity ->> 'kind' IS DISTINCT FROM 'hero' THEN RETURN FALSE;
        END IF;
    END IF;
    IF entity ->> 'kind' = 'dark_arts' AND (
        entity ->> 'zone' NOT IN ('dark_arts_deck', 'dark_arts_discard')
        OR entity ?| ARRAY['owner_position', 'resources', 'resource_limits', 'influence_cost', 'reward_rule_id', 'dark_arts_count']
        OR NOT entity ?& ARRAY['catalog_id', 'effect_rule_id']
    ) THEN RETURN FALSE;
    END IF;
       ...
       OR (
            ...
            AND (
                ...
                OR entity ->> 'kind' NOT IN (
                    'generic', 'hero', 'hogwarts_card', 'starter_card', 'villain', 'location', 'dark_arts'
                )
            )
       )
       OR (
...
CREATE FUNCTION valid_preparation_samples_v6(snapshot JSONB)
...
BEGIN
    IF samples IS NULL THEN
        RETURN snapshot -> 'versions' ->> 'manifest' <> '4';
    END IF;
    IF jsonb_typeof(samples) IS DISTINCT FROM 'array'
       OR jsonb_array_length(samples) <> 40 + 9 * participants
       OR (snapshot -> 'prng' ->> 'counter')::NUMERIC < jsonb_array_length(samples) THEN
        RETURN FALSE;
    END IF;
    FOR pile IN SELECT * FROM (VALUES
        (0, 'hogwarts_deck', NULL::INTEGER, 30),
        (1, 'dark_arts_deck', NULL::INTEGER, 10),
        (2, 'villain_deck', NULL::INTEGER, 3)
...
CREATE FUNCTION valid_game_snapshot_v5(snapshot JSONB)
RETURNS BOOLEAN
LANGUAGE plpgsql
IMMUTABLE
AS $$
...
BEGIN
    IF compact_jsonb_octet_length(snapshot) > 4194304
       OR valid_preparation_samples_v6(snapshot) IS NOT TRUE
       ...
       OR snapshot ->> 'snapshot_version' <> '5'
    ...
    IF snapshot -> 'turn' IS DISTINCT FROM jsonb_build_object(
       ...
       OR valid_engine_control_v6(jsonb_build_object(
            'status', snapshot -> 'status',
       ...
       OR EXISTS (
            ...
            WHERE valid_turn_step_v6(step) IS NOT TRUE
       )
    ...
    IF snapshot -> 'prng' IS DISTINCT FROM jsonb_build_object(
       ...
       OR EXISTS (
            ...
            WHERE valid_snapshot_effect_entity_v5(entity) IS NOT TRUE
       )
       ...
       OR (
            ...
            AND (
                ...
                OR EXISTS (
                    ...
                    WHERE valid_effect_outcome_v6(outcome) IS NOT TRUE
                )
            )
       )
       OR (
            snapshot -> 'effects' ? 'choice'
            AND valid_pending_effect_choice_v6(snapshot -> 'effects' -> 'choice') IS NOT TRUE
       )
    THEN
        RETURN FALSE;
    END IF;
...
CREATE FUNCTION normalized_effect_entities_for_turn_v6(snapshot JSONB)
RETURNS JSONB
LANGUAGE plpgsql
IMMUTABLE
AS $$
...
CREATE FUNCTION change_effect_entity_resource_v6(
    entities JSONB,
    target_id TEXT,
...
CREATE FUNCTION change_drawing_permission_v6(world JSONB, target_id TEXT, owner JSONB, blocked BOOLEAN)
RETURNS JSONB LANGUAGE plpgsql IMMUTABLE AS $$
DECLARE target JSONB; changed JSONB;
BEGIN
    SELECT entity INTO target FROM jsonb_array_elements(world) AS entity WHERE entity ->> 'id' = target_id;
    IF NOT FOUND OR target ->> 'kind' IS DISTINCT FROM 'hero'
        OR target -> 'owner_position' IS DISTINCT FROM owner
        OR COALESCE((target ->> 'drawing_blocked')::BOOLEAN, FALSE) = blocked THEN RETURN NULL;
    END IF;
    SELECT jsonb_agg(CASE WHEN entity ->> 'id' = target_id THEN
        (entity - 'drawing_blocked') || CASE WHEN blocked THEN '{"drawing_blocked":true}'::jsonb ELSE '{}'::jsonb END
        ELSE entity END ORDER BY ordering) INTO changed
    FROM jsonb_array_elements(world) WITH ORDINALITY AS e(entity, ordering);
    RETURN changed;
EXCEPTION WHEN OTHERS THEN RETURN NULL;
END;
$$;

CREATE FUNCTION apply_effect_steps_v6(initial_world JSONB, steps JSONB)
RETURNS JSONB
LANGUAGE plpgsql
IMMUTABLE
AS $$
...
BEGIN
    ...
    LOOP
        CASE effect ->> 'type'
            WHEN 'drawing_blocked' THEN
                world := change_drawing_permission_v6(world, effect ->> 'target_id', effect -> 'target_position', TRUE);
            WHEN 'moved' THEN
...
CREATE FUNCTION valid_choice_world_transition_v6(
...
DECLARE
    world JSONB;
BEGIN
    world := apply_effect_steps_v6(
        normalized_effect_entities_for_turn_v6(previous_snapshot),
        payload -> 'steps'
    );
...
CREATE FUNCTION replay_shuffle_v6(cards TEXT[], samples JSONB)
RETURNS TEXT[] LANGUAGE plpgsql IMMUTABLE AS $$
DECLARE upper_bound INTEGER; sample JSONB; selected INTEGER; swapped TEXT; sample_index INTEGER := 0;
BEGIN
    IF jsonb_typeof(COALESCE(samples, '[]'::jsonb)) IS DISTINCT FROM 'array'
        OR jsonb_array_length(COALESCE(samples, '[]'::jsonb)) <> cardinality(cards) - 1 THEN RETURN NULL;
    END IF;
    FOR upper_bound IN REVERSE cardinality(cards)..2 LOOP
        sample := samples -> sample_index;
        IF sample IS DISTINCT FROM jsonb_build_object('upper_exclusive', upper_bound, 'result', sample -> 'result')
           OR jsonb_typeof(sample -> 'result') IS DISTINCT FROM 'number'
           OR sample ->> 'result' !~ '^(0|[1-9][0-9]*)$'
           OR (sample ->> 'result')::NUMERIC >= upper_bound THEN RETURN NULL;
        END IF;
        selected := (sample ->> 'result')::INTEGER + 1;
        swapped := cards[upper_bound]; cards[upper_bound] := cards[selected]; cards[selected] := swapped;
        sample_index := sample_index + 1;
    END LOOP;
    RETURN cards;
EXCEPTION WHEN OTHERS THEN RETURN NULL;
...
CREATE FUNCTION valid_end_turn_outcome_v6(outcome JSONB)
RETURNS BOOLEAN
LANGUAGE plpgsql
IMMUTABLE
AS $$
BEGIN
    ...
    CASE outcome ->> 'type'
        WHEN 'drawing_restored' THEN
            RETURN outcome = jsonb_build_object('type', 'drawing_restored', 'position', outcome -> 'position')
        ...
        WHEN 'pile_shuffled' THEN
            IF outcome - 'samples' IS DISTINCT FROM jsonb_build_object(
                    'type', outcome -> 'type',
               ...
               OR replay_shuffle_v6(ARRAY(SELECT card #>> '{}' FROM jsonb_array_elements(outcome -> 'bottom_to_top') AS card), outcome -> 'samples') IS NULL
               OR EXISTS (
                    SELECT 1
...
CREATE FUNCTION valid_end_turn_sequence_v6(outcomes JSONB, actor_position INTEGER)
RETURNS BOOLEAN
LANGUAGE plpgsql
IMMUTABLE
AS $$
...
BEGIN
    IF jsonb_typeof(outcomes) IS DISTINCT FROM 'array'
       ...
       OR EXISTS (
            ...
            WHERE valid_end_turn_outcome_v6(outcome) IS NOT TRUE
               OR (
    ...
    WHILE outcomes -> outcome_index ->> 'type' = 'hero_recovered' LOOP
        outcome_index := outcome_index + 1;
    END LOOP;
    WHILE outcomes -> outcome_index ->> 'type' = 'drawing_restored' LOOP
        outcome_index := outcome_index + 1;
    END LOOP;
...
CREATE FUNCTION valid_turn_world_transition_v6(
    previous_snapshot JSONB,
    committed_entities JSONB,
    payload JSONB
)
...
DECLARE
    ...
    resource_name TEXT;
    top_card_id TEXT;
    world JSONB := normalized_effect_entities_for_turn_v6(previous_snapshot);
BEGIN
    IF jsonb_typeof(world) IS DISTINCT FROM 'array'
    ...
    FOR recovered IN SELECT entity FROM jsonb_array_elements(world) AS entity
        ...
        END IF;
        world := change_drawing_permission_v6(world, recovered ->> 'id', recovered -> 'owner_position', FALSE);
        outcome_index := outcome_index + 1;
    END LOOP;
...
CREATE FUNCTION valid_turn_completed_payload_v6(payload JSONB)
RETURNS BOOLEAN
LANGUAGE plpgsql
IMMUTABLE
AS $$
...
BEGIN
    IF compact_jsonb_octet_length(payload) > 4194304
       ...
       OR jsonb_typeof(payload -> 'steps') IS DISTINCT FROM 'array'
       OR valid_engine_control_v6(payload -> 'control') IS NOT TRUE
...
CREATE FUNCTION valid_choice_resolved_payload_v6(payload JSONB)
RETURNS BOOLEAN
LANGUAGE plpgsql
IMMUTABLE
AS $$
...
CREATE FUNCTION valid_hero_action_payload_v6(
    payload JSONB,
...
BEGIN
    ...
    IF relational_event_type IN ('card_played', 'attack_assigned') THEN
        ...
        IF payload ->> 'effect_stop' = 'choice' THEN
            IF valid_pending_effect_choice_v6(payload -> 'choice') IS NOT TRUE THEN
                RETURN FALSE;
            END IF;
...
CREATE FUNCTION merge_turn_steps_v6(previous_steps JSONB, incremental_steps JSONB)
RETURNS JSONB
LANGUAGE plpgsql
IMMUTABLE
AS $$
...
BEGIN
    ...
    IF NEW.sequence <> 0
       ...
       OR valid_game_snapshot_v5(NEW.snapshot) IS NOT TRUE
...
BEGIN
    ...
    IF NEW.event_version <> 6
       ...
       OR (CASE NEW.event_type
            WHEN 'turn_completed' THEN valid_turn_completed_payload_v6(NEW.payload)
            WHEN 'choice_resolved' THEN valid_choice_resolved_payload_v6(NEW.payload)
            ELSE FALSE
          END) IS NOT TRUE
    THEN
...
BEGIN
    ...
    IF NEW.snapshot_version <> 5
       OR OLD.snapshot_version NOT IN (1, 2, 3, 4, 5)
    THEN
    ...
    IF transition_event_type IN ('card_played', 'attack_assigned', 'card_acquired') THEN
        expected_entities := apply_effect_steps_v6(
            normalized_effect_entities_for_turn_v6(OLD.snapshot),
    ...
    CASE transition_event_type
        WHEN 'turn_completed' THEN
            ...
            IF transition_command_type <> 'end_hero_actions'
               ...
               OR valid_turn_world_transition_v6(
                    OLD.snapshot,
        ...
        WHEN 'choice_resolved' THEN
            IF OLD.snapshot_version IN (3, 4, 5)
            ...
            expected_steps := merge_turn_steps_v6(
                previous_steps,
            ...
            IF transition_command_type <> 'resolve_choice'
               ...
               OR OLD.snapshot -> 'turn' ->> 'phase'
                    NOT IN ('dark_arts', 'villains', 'hero_actions')
               OR valid_pending_effect_choice_v6(old_choice) IS NOT TRUE
               OR transition_payload ->> 'turn'
                    IS DISTINCT FROM OLD.snapshot -> 'turn' ->> 'number'
               OR transition_payload ->> 'actor_position'
               ...
               OR NEW.snapshot -> 'turn' -> 'active_position'
                    IS DISTINCT FROM OLD.snapshot -> 'turn' -> 'active_position'
               OR valid_choice_world_transition_v6(
                    OLD.snapshot,
                    NEW.snapshot -> 'effects' -> 'entities',
                    transition_payload
               ) IS NOT TRUE
    ...
    IF valid_game_snapshot_v5(NEW.snapshot) IS NOT TRUE
       OR (NEW.snapshot ->> 'snapshot_version')::NUMERIC <> NEW.snapshot_version
       OR (NEW.snapshot ->> 'state_version')::NUMERIC <> NEW.state_version
       OR (NEW.snapshot ->> 'sequence')::NUMERIC <> NEW.sequence
       OR NEW.snapshot ->> 'status' IS DISTINCT FROM NEW.status
