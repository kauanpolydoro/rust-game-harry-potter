-- Game 2 adds cumulative preparation and persisted ally copies.
-- Game 1 continues to use snapshot 5, event 6 and its original manifest digest.
ALTER TABLE games DROP CONSTRAINT games_snapshot_version_supported,
    ADD CONSTRAINT games_snapshot_version_supported CHECK (snapshot_version IN (1, 2, 3, 4, 5, 6));
ALTER TABLE game_events DROP CONSTRAINT game_events_version_supported,
    ADD CONSTRAINT game_events_version_supported CHECK (event_version IN (1, 2, 3, 4, 5, 6, 7));

CREATE FUNCTION valid_effect_outcome_v7(effect JSONB)
RETURNS BOOLEAN LANGUAGE plpgsql IMMUTABLE AS $$
BEGIN
    IF effect ->> 'type' <> 'ally_copied' THEN RETURN valid_effect_outcome_v6(effect); END IF;
    RETURN effect = jsonb_build_object('type', 'ally_copied', 'rule_id', effect -> 'rule_id',
        'card_id', effect -> 'card_id', 'ally_id', effect -> 'ally_id', 'owner_position', effect -> 'owner_position')
        AND NOT EXISTS (SELECT 1 FROM unnest(ARRAY['rule_id','card_id','ally_id']) AS field
            WHERE jsonb_typeof(effect -> field) IS DISTINCT FROM 'string'
               OR octet_length(effect ->> field) NOT BETWEEN 1 AND 256)
        AND effect ->> 'card_id' <> effect ->> 'ally_id'
        AND jsonb_typeof(effect -> 'owner_position') = 'number'
        AND effect ->> 'owner_position' ~ '^[1-4]$';
EXCEPTION WHEN OTHERS THEN RETURN FALSE;
END;
$$;

CREATE FUNCTION valid_snapshot_effect_entity_v6(entity JSONB)
RETURNS BOOLEAN LANGUAGE sql IMMUTABLE
RETURN valid_snapshot_effect_entity_v5(entity - 'copied_ally_id') IS TRUE
    AND (NOT entity ? 'copied_ally_id' OR (
        entity ->> 'zone' = 'hero_play_area'
        AND entity ->> 'kind' IN ('hogwarts_card','starter_card')
        AND jsonb_typeof(entity -> 'copied_ally_id') = 'string'
        AND octet_length(entity ->> 'copied_ally_id') BETWEEN 1 AND 256
        AND entity ->> 'copied_ally_id' <> entity ->> 'id'
    ));

CREATE FUNCTION record_ally_copy_v7(world JSONB, effect JSONB)
RETURNS JSONB LANGUAGE plpgsql IMMUTABLE AS $$
DECLARE
    card JSONB;
    ally JSONB;
BEGIN
    IF valid_effect_outcome_v7(effect) IS NOT TRUE THEN RETURN NULL; END IF;
    SELECT value INTO card FROM jsonb_array_elements(world) WHERE value ->> 'id' = effect ->> 'card_id';
    SELECT value INTO ally FROM jsonb_array_elements(world) WHERE value ->> 'id' = effect ->> 'ally_id';
    IF card IS NULL OR ally IS NULL
        OR card ->> 'zone' IS DISTINCT FROM 'hero_play_area'
        OR ally ->> 'zone' IS DISTINCT FROM 'hero_play_area'
        OR card -> 'owner_position' IS DISTINCT FROM effect -> 'owner_position'
        OR ally -> 'owner_position' IS DISTINCT FROM effect -> 'owner_position'
        OR card ->> 'effect_rule_id' IS DISTINCT FROM effect ->> 'rule_id'
        OR card ? 'copied_ally_id' OR ally ? 'copied_ally_id'
        OR ally ->> 'effect_rule_id' IS NULL THEN RETURN NULL;
    END IF;
    RETURN (SELECT jsonb_agg(CASE WHEN value ->> 'id' = effect ->> 'card_id'
        THEN value || jsonb_build_object('copied_ally_id', effect -> 'ally_id') ELSE value END ORDER BY position)
        FROM jsonb_array_elements(world) WITH ORDINALITY AS entity(value, position));
EXCEPTION WHEN OTHERS THEN RETURN NULL;
END;
$$;

CREATE FUNCTION move_effect_entity_v7(entities JSONB, target_id TEXT, expected_owner JSONB, from_zone TEXT, to_zone TEXT)
RETURNS JSONB LANGUAGE sql IMMUTABLE
RETURN (SELECT jsonb_agg(CASE WHEN value ->> 'id' = target_id AND to_zone <> 'hero_play_area'
    THEN value - 'copied_ally_id' ELSE value END ORDER BY position)
    FROM jsonb_array_elements(move_effect_entity_v4(entities, target_id, expected_owner, from_zone, to_zone))
        WITH ORDINALITY AS entity(value, position));


CREATE FUNCTION valid_turn_step_v7(step JSONB)
RETURNS BOOLEAN
LANGUAGE plpgsql
IMMUTABLE
AS $$
BEGIN
    ...
    RETURN NOT EXISTS (
        ...
        WHERE valid_effect_outcome_v7(effect) IS NOT TRUE
    );
END;
$$;

CREATE FUNCTION valid_preparation_samples_v7(snapshot JSONB)
RETURNS BOOLEAN LANGUAGE plpgsql IMMUTABLE AS $$
DECLARE
...
BEGIN
    IF samples IS NULL THEN
        RETURN snapshot -> 'versions' ->> 'manifest' <> '5';
    END IF;
    IF jsonb_typeof(samples) IS DISTINCT FROM 'array'
       OR jsonb_array_length(samples) <> 62 + 9 * participants
       OR (snapshot -> 'prng' ->> 'counter')::NUMERIC < jsonb_array_length(samples) THEN
        RETURN FALSE;
    END IF;
    FOR pile IN SELECT * FROM (VALUES
        (0, 'hogwarts_deck', NULL::INTEGER, 44),
        (1, 'dark_arts_deck', NULL::INTEGER, 15),
        (2, 'villain_deck', NULL::INTEGER, 6)
...
CREATE FUNCTION valid_game_snapshot_v6(snapshot JSONB)
RETURNS BOOLEAN
LANGUAGE plpgsql
IMMUTABLE
AS $$
...
BEGIN
    IF snapshot ->> 'snapshot_version' = '5' THEN
        RETURN snapshot -> 'versions' ->> 'manifest' <> '5' AND valid_game_snapshot_v5(snapshot);
    END IF;
    IF snapshot -> 'versions' ->> 'manifest' <> '5' OR snapshot ->> 'adventure_id' <> 'adventure:002' THEN RETURN FALSE; END IF;
    IF compact_jsonb_octet_length(snapshot) > 4194304
       OR valid_preparation_samples_v7(snapshot) IS NOT TRUE
       ...
       OR snapshot ->> 'snapshot_version' <> '6'
    ...
    IF snapshot -> 'turn' IS DISTINCT FROM jsonb_build_object(
       ...
       OR EXISTS (
            ...
            WHERE valid_turn_step_v7(step) IS NOT TRUE
       )
    ...
    IF snapshot -> 'prng' IS DISTINCT FROM jsonb_build_object(
       ...
       OR EXISTS (
            ...
            WHERE valid_snapshot_effect_entity_v6(entity) IS NOT TRUE
       )
       OR EXISTS (
       ...
       OR (
            ...
            AND (
                ...
                OR EXISTS (
                    ...
                    WHERE valid_effect_outcome_v7(outcome) IS NOT TRUE
                )
            )
       )
       OR (
...
CREATE FUNCTION normalized_effect_entities_for_turn_v7(snapshot JSONB)
RETURNS JSONB
LANGUAGE plpgsql
IMMUTABLE
AS $$
...
-- A Local can reveal Petrification twice. Blocking an already blocked Hero
-- preserves the permission; restoring it remains a single validated transition.
CREATE FUNCTION change_drawing_permission_v7(world JSONB, target_id TEXT, owner JSONB, blocked BOOLEAN)
RETURNS JSONB LANGUAGE plpgsql IMMUTABLE AS $$
...
BEGIN
    ...
    IF NOT FOUND OR target ->> 'kind' IS DISTINCT FROM 'hero'
        OR target -> 'owner_position' IS DISTINCT FROM owner THEN RETURN NULL;
    END IF;
    IF blocked AND target -> 'drawing_blocked' = 'true'::jsonb THEN RETURN world; END IF;
    RETURN change_drawing_permission_v6(world, target_id, owner, blocked);
EXCEPTION WHEN OTHERS THEN RETURN NULL;
END;
$$;

CREATE FUNCTION apply_effect_steps_v7(initial_world JSONB, steps JSONB)
RETURNS JSONB
LANGUAGE plpgsql
IMMUTABLE
AS $$
...
CREATE FUNCTION valid_choice_world_transition_v7(
...
DECLARE
    world JSONB;
BEGIN
    world := apply_effect_steps_v7(
        normalized_effect_entities_for_turn_v7(previous_snapshot),
        payload -> 'steps'
    );
...
CREATE FUNCTION valid_turn_world_transition_v7(
    previous_snapshot JSONB,
    committed_entities JSONB,
    payload JSONB
)
...
DECLARE
    ...
    resource_name TEXT;
    top_card_id TEXT;
    world JSONB := normalized_effect_entities_for_turn_v7(previous_snapshot);
BEGIN
    IF jsonb_typeof(world) IS DISTINCT FROM 'array'
...
CREATE FUNCTION valid_turn_completed_payload_v7(payload JSONB)
RETURNS BOOLEAN
LANGUAGE plpgsql
IMMUTABLE
AS $$
DECLARE
    step_count INTEGER;
BEGIN
    IF payload ->> 'event_version' = '6' THEN RETURN valid_turn_completed_payload_v6(payload); END IF;
...
CREATE FUNCTION valid_choice_resolved_payload_v7(payload JSONB)
RETURNS BOOLEAN
LANGUAGE plpgsql
IMMUTABLE
AS $$
DECLARE
    step_count INTEGER;
BEGIN
    IF payload ->> 'event_version' = '6' THEN RETURN valid_choice_resolved_payload_v6(payload); END IF;
    IF compact_jsonb_octet_length(payload) > 4194304
       OR payload IS DISTINCT FROM jsonb_build_object(
            'event_version', payload -> 'event_version',
            'type', payload -> 'type',
...
CREATE FUNCTION valid_hero_action_payload_v7(
    payload JSONB,
...
DECLARE
    expected JSONB;
BEGIN
    IF payload ->> 'event_version' = '6' THEN RETURN valid_hero_action_payload_v6(payload, relational_event_type, relational_sequence, relational_state_version, relational_actor_position, committed_prng_counter, committed_status); END IF;
...
CREATE FUNCTION merge_turn_steps_v7(previous_steps JSONB, incremental_steps JSONB)
RETURNS JSONB
LANGUAGE plpgsql
IMMUTABLE
AS $$
...
BEGIN
    ...
    IF NEW.sequence <> 0
       ...
       OR valid_game_snapshot_v6(NEW.snapshot) IS NOT TRUE
...
BEGIN
    ...
    IF NEW.event_version IN (6, 7) AND NEW.event_version <> (CASE WHEN committed_snapshot ->> 'snapshot_version' = '6' THEN 7 ELSE 6 END) THEN
    ...
    IF NEW.event_version NOT IN (6, 7)
       OR NEW.event_type NOT IN ('turn_completed', 'choice_resolved')
       OR (CASE NEW.event_type
            WHEN 'turn_completed' THEN valid_turn_completed_payload_v7(NEW.payload)
            WHEN 'choice_resolved' THEN valid_choice_resolved_payload_v7(NEW.payload)
            ELSE FALSE
          END) IS NOT TRUE
    THEN
...
BEGIN
    ...
    IF NEW.snapshot_version <> (CASE WHEN NEW.manifest_version = 5 THEN 6 ELSE 5 END)
       OR OLD.snapshot_version NOT IN (1, 2, 3, 4, 5, 6)
    THEN
    ...
    WHERE events.game_id = NEW.id
      ...
      AND events.event_version = CASE WHEN NEW.snapshot_version = 6 THEN 7 ELSE 6 END;
    ...
    IF transition_event_type IN ('card_played', 'attack_assigned', 'card_acquired') THEN
        expected_entities := apply_effect_steps_v7(
            normalized_effect_entities_for_turn_v7(OLD.snapshot),
        ...
        IF OLD.status <> 'in_progress'
           ...
           OR (
                OLD.snapshot_version IN (3, 4, 5, 6)
    ...
    CASE transition_event_type
        WHEN 'turn_completed' THEN
            ...
            IF transition_command_type <> 'end_hero_actions'
               ...
               OR valid_turn_world_transition_v7(
                    OLD.snapshot,
        ...
        WHEN 'choice_resolved' THEN
            IF OLD.snapshot_version IN (3, 4, 5, 6)
            ...
            expected_steps := merge_turn_steps_v7(
                previous_steps,
            ...
            IF transition_command_type <> 'resolve_choice'
               ...
               OR NEW.snapshot -> 'turn' -> 'active_position'
                    IS DISTINCT FROM OLD.snapshot -> 'turn' -> 'active_position'
               OR valid_choice_world_transition_v7(
                    OLD.snapshot,
                    NEW.snapshot -> 'effects' -> 'entities',
                    transition_payload
               ) IS NOT TRUE
    ...
    IF valid_game_snapshot_v6(NEW.snapshot) IS NOT TRUE
       OR (NEW.snapshot ->> 'snapshot_version')::NUMERIC <> NEW.snapshot_version
       OR (NEW.snapshot ->> 'state_version')::NUMERIC <> NEW.state_version
       OR (NEW.snapshot ->> 'sequence')::NUMERIC <> NEW.sequence
       OR NEW.snapshot ->> 'status' IS DISTINCT FROM NEW.status
