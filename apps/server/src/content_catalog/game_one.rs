...
/// Loads the cumulative Game 4 adaptation with versioned House dice.
///
/// # Panics
/// Panics if the shipped bundle is invalid, unsupported, or incomplete.
#[must_use]
pub fn game_four_manifest() -> ContentManifest {
    adventure_manifest(super::GamePreparation::Four)
}

fn adventure_manifest(game: super::GamePreparation) -> ContentManifest {
    use super::GamePreparation;
    let bytes: &[u8] = match game {
        GamePreparation::Four => include_bytes!("../../../../content/bundles/game-four-en-v1.json"),
    ...
    let inspect = match game {
        GamePreparation::Four => game_content::inspect_game_four_rules,
    ...
    let import = match game {
        GamePreparation::Four => game_content::import_game_four_bundle_with_runtime_rules,
    ...
    let sources = ["one", "two", "three", "four"].into_iter().take(match game { GamePreparation::One => 1, GamePreparation::Two => 2, GamePreparation::Three => 3, GamePreparation::Four => 4 }).map(|game| ProvenanceSource {
