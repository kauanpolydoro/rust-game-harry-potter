...
#[tokio::test]
async fn migration_0013_upgrades_existing_recovery_credentials() {
    ...
    assert_eq!(observation.pre_upgrade_applied_version, 11);
    assert_eq!(observation.pre_upgrade_schema_version, "11");
    ...
    assert_eq!(observation.applied_version, 13);
    assert_eq!(observation.schema_version, "13");
}

async fn exercise_upgrade(database: &PgPool) -> TestResult<UpgradeObservation> {
    // There is no version 12 file, so this target selects the exact schema immediately before 0013.
    MIGRATOR.run_to(12, database).await?;
