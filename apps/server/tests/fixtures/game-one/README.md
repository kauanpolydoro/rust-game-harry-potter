...
O teste `game_one_browser_transcripts_replay_exact_events_and_snapshot_goldens` usa o manifesto publicado e os codecs de produção.
...
```sh
cargo test -p harry-potter-server --lib game_one_browser_transcripts
```
...
```sh
UPDATE_GAME_ONE_GOLDENS=1 cargo test -p harry-potter-server --lib game_one_browser_transcripts
```
...
Os testes de navegador exportam `game-one-commands.json` como artefato para comparar ou substituir uma transcrição de entrada quando o cenário mudar intencionalmente.
